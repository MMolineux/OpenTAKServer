...
if [[ "$1" == "clean" ]]; then
  ...
  if [[ -z "$service" ]] && command -v tmux &>/dev/null; then
    ...
    # # server
    tmux send-keys -t "$SESSION" 'export $(cat .env.dev | grep -v '^#' | xargs); poetry --directory ./server run opentakserver' C-m

    # workers window with two panes
    tmux new-window -t "$SESSION" -n workers
    tmux split-window -t "$SESSION:workers" -h
    tmux send-keys -t "$SESSION:workers.0" 'export $(cat .env.dev | grep -v '^#' | xargs); poetry --directory ./server run eud_handler' C-m
    tmux send-keys -t "$SESSION:workers.1" 'export $(cat .env.dev | grep -v '^#' | xargs); poetry --directory ./server run cot_parser' C-m
