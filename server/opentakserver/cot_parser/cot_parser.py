...
from opentakserver.models.MissionChange import (
    MissionChange,
...
class CoTController:
    ...
    def run(self):
        ...
        self.rabbit_channel.queue_bind(
            exchange="cot_parser", queue="cot_parser", routing_key="cot_parser"
        )
        self.rabbit_channel.basic_qos(
            prefetch_count=self.context.app.config.get("OTS_RABBITMQ_PREFETCH")
        )
        self.rabbit_channel.basic_consume(
            queue="cot_parser", on_message_callback=self.on_message, auto_ack=False
        )
        ...
        if point and not point.attrs["lat"].startswith("999"):
            ...
            with self.context:
                ...
                self.db.session.execute(
                    update(MissionUID)
                    .where(MissionUID.uid == event.attrs["uid"])
                    .values(
                ...
                p = self.db.session.execute(
                    select(Point).filter(Point.id == res.inserted_primary_key[0])
                ).first()[0]
                ...
                if self.context.app.config.get("OTS_ENABLE_MESHTASTIC"):
                    try:
                        ...
                        if eud.last_meshtastic_publish is None or (
                            now
                            - eud.last_meshtastic_publish.replace(tzinfo=timezone.utc)
                        ).total_seconds() >= self.context.app.config.get(
                            ...
                            try:
                                ...
                                self.db.session.execute(
                                    update(EUD)
                                    .filter_by(uid=eud.uid)
                                    .values(last_meshtastic_publish=now)
                                )
                                self.db.session.commit()
                                ...
                                if eud.platform != "Meshtastic":
                                    ...
                                    setattr(
                                        mesh_user,
                                        "id",
                                        "!{:x}".format(eud.meshtastic_id),
                                    )
                                    mesh_user.hw_model = (
                                        mesh_pb2.HardwareModel.PRIVATE_HW
                                    )
                                    ...
                                    if (
                                        now
                                        - eud.last_meshtastic_publish.replace(
                                            tzinfo=timezone.utc
                                        )
                                    ).total_seconds() >= self.context.app.config.get(
                                        "OTS_MESHTASTIC_NODEINFO_INTERVAL"
                                    ) * self.context.app.config.get(
                                        "OTS_MESHTASTIC_PUBLISH_INTERVAL"
                                    ):
                                        ...
                                        encoded_message.portnum = (
                                            portnums_pb2.NODEINFO_APP
                                        )
                                        encoded_message.payload = (
                                            mesh_user.SerializeToString()
                                        )
                                        self.publish_to_meshtastic(
                                            self.get_protobuf(
                                                encoded_message, uid=p.device_uid
                                            )
                                        )

                                    position = mesh_pb2.Position()
                                    position.latitude_i = int(p.latitude / 0.0000001)
                                    position.longitude_i = int(p.longitude / 0.0000001)
                                    position.altitude = int(p.hae)
                                    position.timestamp = int(
                                        time.mktime(p.timestamp.timetuple())
                                    )
                                    position.ground_track = (
                                        int(p.course) if p.course else 0
                                    )
                                    position.ground_speed = (
                                        int(p.speed) if p.speed and p.speed >= 0 else 0
                                    )
                                    ...
                                    tak_packet.contact.device_callsign, size = (
                                        unishox2.compress(eud.uid)
                                    )
                                    tak_packet.contact.callsign, size = (
                                        unishox2.compress(eud.callsign)
                                    )
                                    tak_packet.group.team = (
                                        eud.team.name.replace(" ", "_")
                                        if eud.team
                                        else "Cyan"
                                    )
                                    tak_packet.group.role = (
                                        eud.team_role.replace(" ", "")
                                        if eud.team_role
                                        else "TeamMember"
                                    )
                                    tak_packet.status.battery = (
                                        int(p.battery) if p.battery else 0
                                    )
                                    tak_packet.pli.latitude_i = int(
                                        p.latitude / 0.0000001
                                    )
                                    tak_packet.pli.longitude_i = int(
                                        p.longitude / 0.0000001
                                    )
                                    tak_packet.pli.altitude = int(p.hae) if p.hae else 0
                                    tak_packet.pli.speed = (
                                        int(p.speed) if p.speed else 0
                                    )
                                    tak_packet.pli.course = (
                                        int(p.course) if p.course else 0
                                    )
                                ...
                                self.logger.error(
                                    f"Failed to send publish message to mesh: {e}"
                                )
    ...
    def get_protobuf(
        ...
        if uid and not from_id:
            try:
                ...
                self.logger.error(
                    "Failed to find EUD {}, using random Meshtastic ID".format(uid)
                )
                self.logger.debug(traceback.format_exc())
        ...
        if chat:
            ...
            if self.context.app.config.get("OTS_ENABLE_MESHTASTIC"):
                try:
                    with self.context:
                        ...
                        if (
                            dest
                            and "callsign" in dest.attrs
                            and dest.attrs["callsign"] == chat.attrs["chatroom"]
                        ):
                            # This is a DM
                            to = chat.attrs["id"]
                            try:
                        ...
                        if send_meshtastic_text:
                            ...
                            self.publish_to_meshtastic(
                                self.get_protobuf(
                                    encoded_message,
                                    to_id=to_id,
                                    from_id=from_eud.meshtastic_id,
                                )
                            )
                        else:
            ...
            for attr in chat_group.attrs:
                if attr.startswith("uid") and attr != "uid":
                    if (
                        "groupOwner" in chat.attrs
                        and chat.attrs["groupOwner"].lower() == "true"
                        and attr == "uid0"
                    ):
                        with self.context:
                            self.db.session.execute(
                                update(Chatroom)
                                .where(Chatroom.id == chat.attrs["id"])
                    ...
                    with self.context:
                        try:
                            ...
                            self.db.session.commit()
                            self.logger.debug(
                                "add {} to chatroom {}".format(
                                    chatroom_uid.uid, chatroom_uid.chatroom_id
                                )
                            )
                        except exc.IntegrityError:
                            self.db.session.rollback()
    ...
    def parse_video(self, event, cot_pk):
        ...
        if video:
            ...
            v.ignore_embedded_klv = (
                connection_entry.attrs["ignoreEmbeddedKLV"].lower() == "true"
            )
            ...
            with self.context:
                try:
                    ...
                    self.db.session.execute(
                        update(VideoStream)
                        .where(VideoStream.uid == connection_entry.attrs["uid"])
                        .values(
                            network_timeout=connection_entry.attrs["networkTimeout"],
                            ...
                            ignore_embedded_klv=(
                                connection_entry.attrs["ignoreEmbeddedKLV"].lower()
                                == "true"
                            ),
            ...
            if "type" in emergency.attrs:
                ...
                with self.context:
                    try:
                        ...
                        alert.cancel_time = datetime_from_iso8601_string(
                            event.attrs["start"]
                        )
                        self.db.session.commit()
                        self.socketio.emit(
                            "alert", alert.to_json(), namespace="/socket.io"
                        )
                    except BaseException as e:
                        self.logger.error(
                            "Failed to set alert cancel time: {}".format(e)
                        )
                        self.logger.debug(traceback.format_exc())
    ...
    def parse_casevac(self, event, uid, point_pk, cot_pk):
        ...
        if medevac:
            ...
            with self.context:
                ...
                try:
                    self.db.session.execute(
                        insert(CasEvac).values(
                            timestamp=datetime_from_iso8601_string(
                                event.attrs["start"]
                            ),
                            sender_uid=uid,
                            uid=event.attrs["uid"],
                    ...
                    if zmist:
                        self.db.session.execute(
                            insert(ZMIST).values(
                                casevac_uid=event.attrs["uid"], **zmist.attrs
                            )
                        )
                except exc.IntegrityError as e:
                    self.db.session.rollback()
                ...
                try:
                    casevac: CasEvac = self.db.session.execute(
                        self.db.session.query(CasEvac).filter_by(uid=event.attrs["uid"])
                    ).first()[0]
                    self.socketio.emit(
                        "casevac", casevac.to_json(), namespace="/socket.io"
                    )
                except BaseException as e:
    ...
    def parse_marker(self, event, uid, point_pk, cot_pk):
        if (
            ...
            try:
                ...
                if link:
                    marker.parent_callsign = (
                        link.attrs["parent_callsign"]
                        if "parent_callsign" in link.attrs
                        else None
                    )
                    marker.production_time = (
                        link.attrs["production_time"]
                        if "production_time" in link.attrs
                        else iso8601_string_from_datetime(datetime.now(timezone.utc))
                    )
                    marker.relation = (
                        link.attrs["relation"] if "relation" in link.attrs else None
                    )
                    marker.relation_type = (
                        link.attrs["relation_type"]
                        if "relation_type" in link.attrs
                        else None
                    )
                    marker.parent_uid = (
                        link.attrs["uid"] if "uid" in link.attrs else None
                    )
                else:
                    marker.production_time = iso8601_string_from_datetime(
                ...
                with self.context:
                    try:
                        ...
                        marker = self.db.session.execute(
                            self.db.session.query(Marker).filter(
                                Marker.uid == marker.uid
                            )
                        ).first()[0]

                    self.socketio.emit(
                        "marker", marker.to_json(), namespace="/socket.io"
                    )
    ...
    def parse_rbline(self, event, uid, point_pk, cot_pk):
        if re.match("^u-rb", event.attrs["type"]):
            ...
            if detail:
                ...
                with self.context:
                    start_point = self.db.session.execute(
                        self.db.session.query(Point).filter(Point.id == point_pk)
                    ).first()[0]
                    ...
                    try:
                        ...
                        self.db.session.commit()
                        self.logger.debug(
                            "Inserted new R&B line: {}".format(rb_line.uid)
                        )
                    except exc.IntegrityError:
                        self.db.session.rollback()
                        self.db.session.execute(
                            update(RBLine)
                            .where(RBLine.uid == rb_line.uid)
                    ...
                    self.socketio.emit(
                        "rb_line", rb_line.to_json(), namespace="/socket.io"
                    )
    ...
    def publish_to_meshtastic(self, body):
        for channel in self.get_meshtastic_channels():
            ...
            routing_key = "{}.2.e.{}.outgoing".format(
                self.context.app.config.get("OTS_MESHTASTIC_TOPIC"), channel
            )
    ...
    def get_meshtastic_channels(self):
        with self.context:
            channels = self.db.session.execute(
                self.db.session.query(MeshtasticChannel)
            ).scalars()
            downlink_channels = []
                ...
                if event.attrs["type"] == "t-x-d-d":
                    try:
                        with self.context:
                            ...
                            if eud:
                                eud = eud[0]
                                eud.last_event_time = datetime_from_iso8601_string(
                                    event.attrs["start"]
                                )
                                ...
                                self.socketio.emit(
                                    "eud", eud.to_json(), namespace="/socket.io"
                                )
                    except BaseException as e:
